...
var $perwakilan = 'Nama Perwakilan';
var $user = 'user';
...
function PDF($orientation='P',$unit='mm',$format='A4'){
	...
	$this->SetAuthor('Edisposisi');
	$this->AliasNbPages();
}

function set_var($nama_perwakilan,$user){
	$this->perwakilan = $nama_perwakilan;	
	$this->user = $user;	
}
...
function Header(){
	...
	$user = $this->getvar('UserName');
	$time = date('d/m/Y');
	$edit = $this->getstr('EditBy');
	$date = $this->getstr('Updated');
	$input = "Dicetak oleh ".$this->user." pada tanggal ".date('Y-m-d h:i:s');
	$print = "";
	$print = $edit? "$input. $print":$print;
	$html = "
	<table width=$this->width><tr>
		<td nowrap>".strtoupper($this->perwakilan)." </td>
...
function Footer(){
	...
	$page = 'Halaman '.$this->PageNo().'/{nb}';
	$html = "
	<table width=$this->width><tr>
		<td nowrap>Aplikasi E-Disposisi Perwakilan RI Versi 1.0</td>
